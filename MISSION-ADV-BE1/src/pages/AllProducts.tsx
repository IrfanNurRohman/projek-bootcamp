...
const handleFilterChange = (newFilters) => {
  ...
  const fetchCourses = async () => {
    try {
      const res = await axios.get("http://localhost:8000/api/produk-kelas");
      setCourses(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formMode === "add") {
        await axios.post("http://localhost:8000/allproduct", form);
        alert("✅ Kelas berhasil ditambahkan!");
      } else if (formMode === "edit" && editId) {
        await axios.put(`http://localhost:8000/allproduct/${editId}`, form);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:8000/allproduct/${id}`);
