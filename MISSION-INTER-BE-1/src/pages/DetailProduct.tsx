...
const DetailProduct = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const resCourse = await axios.get(`http://localhost:5000/api/produk-kelas/${id}`);
        setCourse(resCourse.data);

        const resModules = await axios.get(`http://localhost:5000/api/produk-kelas/${id}/detail`);
        setModules(resModules.data);

        const resRelated = await axios.get(`http://localhost:5000/api/produk-kelas`);
