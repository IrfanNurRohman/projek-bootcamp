...
const AllProducts = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (formMode === "add") {
      ...
      setCourses((prev) => prev.map((c, i) => (i === editIndex ? { ...form } : c)));
    }
    setIsFormOpen(false);
  };

  // Helper to format price as you originally used (expects number in thousands)
  const updatePrice = (value) => {
    // Allow typing either raw text or number; if number provided (e.g., 250), convert to `Rp 250 K`
  ...
  return (
    <div>
      ...
      {/* Drawer/Modal Form for Add/Edit */}
      {isFormOpen && (
